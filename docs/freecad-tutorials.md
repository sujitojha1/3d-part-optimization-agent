...
| Accessed | 2026-09-17 |
| Status | Reference input. **Superseded in part (2026-09-17):** requirements v0.5 returns FreeCAD to the stack as the only CAD tool, so §6.1 ("D-04 stands") is historical, and so are the remarks in §3 and §7 about the slicer route and D-11: manufacturability is now the CAM Workbench ([freecad-cam-workbench.md](freecad-cam-workbench.md)). The section 7 changes are folded into [plan.md](plan.md) v0.5 |
