...
| **Second-order meshing inverts elements at small fillet radii** | The agent's own `fillet_radius` edit fails at mesh time, not solve time | M2.3 meshed at the minimum radius on 21 Sep and **nothing inverted** ([record](ge-bracket-mesh.md) section 6), so the risk is not observed on this part within `PARAMS`. D-24 still retries once with `SecondOrderLinear`, then marks the candidate `unverified` — and the retry's own quality numbers are not acceptance evidence |
