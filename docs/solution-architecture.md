...
```
 ...
 │      runs as a subprocess in vendor/fem-env (FreeCAD 1.1.3)  │
...
**Why L2 is a separate process.** FreeCAD brings its own Python and binary dependencies (conda-forge, D-17), so it cannot share the harness's `uv` environment. The L1 capability starts the pipeline in `vendor/fem-env` and reads a JSON result back. The process boundary is also where errors-as-values is enforced: a crashed FreeCAD becomes an `unverified` result, not an exception in the harness.
...
```
face predicates  ──▶  FEM Mesh Groups  ──▶  Gmsh physical groups  ──▶  .inp element sets
                                                          │
peak element  ──▶  which set contains it?  ──▶  one label │
```

Faces are found by geometric predicate after every recompute, never by stored index (D-04), because a parameter change can renumber them. The agent names a region; the verifier does a set-membership test. No geometry reasoning at scoring time, no fuzzy spatial matching, no tolerance to argue about.

### One internal unit system

L2 works in one unit system throughout — the SimJEB frame's N, mm, MPa, tonne/mm³, which is also FreeCAD's CalculiX convention — and **converts only at the boundary**. No conversion happens inside the pipeline. Task intake validates declared units and rejects mismatches by field (`REQ-IN-002`); the cantilever validation case then catches anything that slipped through, because a Pa/MPa error shows up there as a factor of 10⁶ rather than a plausible-looking drift.
...
The agenda for the next pass. None block M1. Questions 1, 2 and 6 are answered by **M2**'s hand pass — by experiment and measurement, not by argument — and 4, 5 and 7 stay open into M3.

1. **Does the agent see the numbers before or after it reads the picture?** If `max_vm` is in context first, the model may anchor on it and the contour becomes decorative — which would quietly hollow out the project's central claim while every metric still looks fine. A two-step exchange (read image → commit the region claim → then reveal numerics) would preserve the thesis and make the region claim independently scoreable. This is the most consequential open question here. → **M2.7** settles it by running the same contour both ways and comparing the region claim. **Answered 22 Sep ([record](m2-exchange.md) §3):** the numbers moved 0 of 18 region claims, so anchoring was not observed. The opposite failure was: once numbers arrive in a second turn, the model writes the governing value into its own claimed region (5 of 9), which is false here. So the exchange is two turns (claim committed first), and the agent's prose is never taken as the location of a number — that comes from the D-06 label on the pipeline's peak.
2. **Where does the proposal step live** — a planner node, or a capability with a typed request? A capability makes the single-edit rule trivially enforceable at the seam; a planner node keeps the graph honest about who decided what. → Decided in **M2.9**'s integration spec, against a real transcript rather than in the abstract.
...
6. **Does the agent get code-editing capability at all?** If not — and this project mostly calls tools — the cleanest move is to disable those capabilities explicitly rather than register them and lean on the guard. → **M2.9** names the answer, since the integration spec enumerates every capability the exchange actually needs.
7. **Multi-part generalisation.** `ge_bracket` only in v1, with `L_bracket` as fallback; a shared part base is not needed until a second part exists.
