...
**What v0.5 changed, in one paragraph.** Four owner decisions on 2026-09-17: the project is built and run on **macOS arm64 only**; **3 Oct is fixed**; a **parametric GE-style jet engine bracket is the demo part**, replacing `L_bracket`; and **the intent's stack is kept — FreeCAD, Gmsh, CalculiX, PrusaSlicer — with no other CAD software**, reversing v0.3's switch to CadQuery. D-02 to D-04, D-06, D-08, D-09, D-11, D-12, D-14 to D-17 and D-22 follow from those, and D-23 and D-24 are new. The mesh-quality and singularity content comes from [fem-geometry-preparation.md](fem-geometry-preparation.md) via [freecad-tutorials.md §7](freecad-tutorials.md) and [fem-workbench.md §9.8](fem-workbench.md).
...
| D-09 | **Material library:** closed JSON file, **5 metal-AM alloys** — Ti-6Al-4V, AlSi10Mg, 316L, 17-4PH, Inconel 718. Fields per D-10. Provenance is a cited handbook or public datasheet per record (not MatWeb). Ti-6Al-4V uses E = 113.8 GPa and ν = 0.342, matching SimJEB, and one cited density. | Finite and enumerable, so "no material in the library works" is provable by exhaustion. v0.4's list included brass, cast iron and 1018 steel, which the D-11 process cannot build — a material-led task could then "win" with an unprintable part. |
...
| D-11 | **Manufacturing check:** one **`metal_am` profile** — maximum unsupported overhang **45°** and minimum wall **1.27 mm**, build direction +z with the base on the plate. **Overhang via PrusaSlicer 2.9.6 CLI**: FreeCAD exports the candidate as STL, PrusaSlicer slices it with a pinned config (support threshold 45°, supports on), and the check fails if the G-code carries any support-material extrusion. **Minimum wall via FreeCAD geometry**: a ray cast inward along each face-tessellation normal on the `Part` shape, the distance to the next surface taken as local thickness. Draft is dropped. CAM is not used. | The intent names PrusaSlicer or a CAM check; the part is additive, so the slicer is the fitting one, and it replaces v0.4's hand-written overhang code. PrusaSlicer models FDM, so it is a **proxy** for laser powder-bed support needs — a stated limitation, not a hidden one. Slicers do not report a minimum wall reliably, so that rule stays geometric. 1.27 mm is the challenge's minimum feature; 45° is a common LPBF guideline and a project choice. |
...
| D-13 | **Budget:** 8 candidate evaluations or 20 minutes wall clock per run, whichever comes first. Per-tool timeout 180 s. In-flight work finishes; no new evaluation launches. | Bounds the run. If M2.5 measures a nominal solve over 60 s, the levers apply in order: D-23 half model, D-24 minimum size, then coarser base size. Coarsening is last because it degrades the field the vision step reads. |
...
| D-16 | **Mutation corpus:** 3 categories × 1 instance = **3 mutants**, each paired with a valid control. Sharp corner: `arm_root_fillet = 0`. Wrong load direction: LC1 sign-flipped. Unit mismatch: the Ti-6Al-4V card in Pa where MPa is declared. **Should:** a second instance per category — LC3 at the challenge's own published error (30° from horizontal instead of 42° from vertical), a sharp pocket corner, and density in g/cm³ declared as kg/m³. | The three categories the intent names, runnable every commit. Paired controls make the false-positive rate reportable. Halved from v0.4: first item on the plan's cut list, taken now. |
| D-17 | **Environment: macOS arm64** (Apple Silicon). Two pinned environments and one pinned app: **(1) FEM environment** — conda-forge `freecad` 1.1.3 (Python 3.11), `gmsh` 4.15.2, `calculix` 2.23, `pyvista` and `ccx2paraview`, installed into gitignored `vendor/fem-env` by a pinned `micromamba` binary from an **explicit lock** (every package URL and SHA-256). **(2) Harness environment** — the S17Code harness under its own `uv` lockfile. **(3) PrusaSlicer 2.9.6** — the signed macOS app, fetched by pinned URL and SHA-256 into `vendor/`. The harness calls the pipeline as a subprocess in the FEM environment. One documented entry command installs all three. Windows is deferred. | Owner decisions 2026-09-17: this Mac only, and the intent's stack. FreeCAD brings its own Python and binary dependencies, so it cannot share a `uv` environment with the harness; a process boundary at the L1 seam is also where errors-as-values already sits. All three FEM packages exist for `osx-arm64` on conda-forge (checked 2026-09-17). |
...
| D-20 | **Protected paths:** `verifiers/**`, `tasks/**`, `materials/**`, and `mutations/**` join the harness defaults (`tests/**`, `conftest.py`, `.github/**`). The solver is exposed as a capability, not added to the command allowlist. Code-editing capabilities are disabled. | The judge must be unreachable by the thing being judged: an agent asked to reduce mass that can edit `tasks/` will lower the safety factor instead of removing material. |
...
| PrusaSlicer or a CAM check; "machinable or printable" | PrusaSlicer as an overhang proxy for metal AM, plus a geometric minimum-wall rule; no CAM | D-11 |
| Manufacturability includes draft | Draft dropped — not applicable to AM | D-11 |
| "Any material in the library" | A closed library of 5 printable alloys | D-09 |

FreeCAD, Gmsh and CalculiX are used as the intent names them. v0.3's substitution of CadQuery for FreeCAD is withdrawn.
...
| REQ-VER-003 | When a candidate is evaluated, the system shall decide manufacturing pass using the task's `metal_am` profile. | Must | Fixtures violating minimum wall and overhang each fail the matching rule; a PrusaSlicer failure or missing G-code is `invalid`, never a pass. |
...
| REQ-DEL-003 | The distribution shall pin every tool dependency — the D-17 FEM explicit lock, the harness `uv` lockfile, and the PrusaSlicer app by URL and SHA-256 — and provide one documented command that runs a bundled task end to end on a clean macOS arm64 machine. | Must | Fresh macOS user account with no Python, conda, FreeCAD, Gmsh, CalculiX or PrusaSlicer present: install and run with no undeclared preinstalled dependency. |
...
- [FEM geometry preparation](fem-geometry-preparation.md), [FreeCAD tutorials](freecad-tutorials.md), [FEM Workbench](fem-workbench.md) — the stack's own documentation; sources for D-04, D-12, D-22, D-23 and D-24.
...
| 0.5 | 2026-09-17 | Owner decisions: macOS arm64 only, 3 Oct fixed, GE-style bracket as the demo part, and the intent's stack kept. D-04 returns to FreeCAD 1.1.3 with spreadsheet parameters and predicate face selection; D-02/D-03 drive Gmsh and CalculiX through FreeCAD; D-11 uses PrusaSlicer for overhang; D-17 is one conda-forge explicit lock plus the harness `uv` lock plus PrusaSlicer. D-08 LC1; D-09 five AM alloys; D-12 calibrated on the demo part; D-14/D-15 one part, 3 tasks; D-16 3 mutants; D-22 V1 on FreeCAD's cantilever, V3 promoted, V2 Should; D-23 and D-24 added; REQ-OPT-008 added. `OD-A` resolved. Section 2.1 records departures from intent. Milestones renamed M1–M6 |
